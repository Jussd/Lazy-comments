...
### 1. [Скачай установщик](https://github.com/privatekey7/Vox/releases/tag/v1.1.0)
...
При первом запуске Vox откроет окно **«Выбор модели»** со списком доступных движков распознавания. Все модели работают офлайн, на твоём компьютере.
...
Выбери одну модель, нажми **«Продолжить»** — Vox её скачает (от 30 секунд до пары минут, зависит от модели и интернета). Модель можно сменить в любой момент из трея.
...
Vox умеет работать с несколькими офлайн-движками. Каталог встроен — выбирай и качай прямо из приложения:
...
| **GigaAM v3 (без пунктуации)** | 215 МБ | RU | Если хочется единый стиль пунктуации от Vox |
...
>
> Модели лежат в `%APPDATA%\vox\models\`, активная сохраняется в `%APPDATA%\vox\config.json` как поле `active_model`.
