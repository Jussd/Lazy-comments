...
В любом окне: чат, браузер, блокнот, поле поиска, терминал, IDE, CLI. Всё работает офлайн, на твоём компьютере, без отправки голоса куда-либо.
...
🔤 Перевод выполняется с использованием API DeepL, обеспечивающего высокое качество перевода.
🔒 Приватность — распознавание речи выполняется локально на устройстве, голосовые данные не передаются третьим лицам.
